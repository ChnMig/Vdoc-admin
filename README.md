...
For local packaging, run `pnpm build` followed by `pnpm release:package v0.3.0`, substituting the intended version. Generated archives stay in the ignored `.artifacts/release/` directory.
...
Verified Linux amd64/arm64 images are published to `ghcr.io/chnmig/vdoc-admin:<tag>`. Use the [single-file Compose deployment](https://chnmig.github.io/Vdoc-site/en/deployment) to run Admin with Backend and persistent dependencies. Release assets include the image digest and source commit in `container-image.json`.
